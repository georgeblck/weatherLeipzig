...
subWeather <- weatherData %>% filter(Year >= since.year) %>% select(c("Month", "Day", 
  "Year", "YDay", "AVGTemp", "MaxTemp2m", "MinTemp2m", "SonnenscheinStunden", "NiederschlagMM"))
colnames(subWeather) <- c("Month", "Day", "Year", "yearDay", "Temp", "MaxTemp", "MinTemp", 
  "SunHours", "PrecipinMM")

# Get y-Axis Limits
ylim.plot <- c(floor(min(subWeather$MinTemp, na.rm = TRUE)/5) * 5, ceiling(max(subWeather$MaxTemp, 
  na.rm = TRUE)/5) * 5)
...
dailyPlot <- dailyPlot + coord_cartesian(ylim = ylim.plot) + scale_y_continuous(breaks = ylim.ticks, 
  labels = a, expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0), breaks = c(15, 
  45, 75, 105, 135, 165, 195, 228, 258, 288, 320, 350), labels = c("Januar", "Februar", 
...
cumWeather <- weatherData %>% filter(Year >= since.year, Year < what.year) %>% group_by(Year) %>% 
  arrange(YDay) %>% mutate(cumPrecip = cumsum(NiederschlagMM), cumSun = cumsum(SonnenscheinStunden)) %>% 
  ungroup() %>% group_by(YDay) %>% summarise_at(c("cumPrecip", "cumSun", "NiederschlagMM"), 
  list(mean = mean, max = max), na.rm = TRUE)
cumWeather <- weatherData %>% filter(Year == what.year) %>% full_join(cumWeather, 
  by = "YDay") %>% arrange(YDay) %>% mutate(cumPrecipYear = cumsum(NiederschlagMM), 
  cumSunYear = cumsum(SonnenscheinStunden)) %>% mutate(precipDiff = cumPrecipYear - 
  cumPrecip_mean, sunDiff = cumSunYear - cumSun_mean, precipMax = NiederschlagMM > 
  NiederschlagMM_max)
ggplot(cumWeather, aes(x = YDay, y = precipDiff/20)) + theme_tufte() + geom_hline(yintercept = 0) + 
  xlab("Jahrestag") + ylab("Abweichung im Niederschlag") + geom_line() + geom_point(data = cumWeather[cumWeather$precipMax == 
  TRUE, ], colour = "blue")

cumWeather %>% ggplot(aes(x = YDay, y = sunDiff)) + theme_tufte() + geom_hline(yintercept = 0) + 
  xlab("Jahrestag") + ylab("Abweichung im Niederschlag") + geom_line()
