...
# Plot the colour gradient for rainfall
...
dailyPlot <- dailyPlot + geom_linerange(recordHighLowPast, mapping = aes(x = yearDay, ymin = low, 
    ymax = high), colour = "wheat2", alpha = 0.9)
...
dailyPlot <- dailyPlot + geom_hline(yintercept = -25, colour = "white", linetype = 1) + geom_hline(yintercept = -20, 
    colour = "white", linetype = 1) + geom_hline(yintercept = -15, colour = "white", 
    linetype = 1) + geom_hline(yintercept = -10, colour = "white", linetype = 1) + 
    geom_hline(yintercept = -5, colour = "white", linetype = 1) + geom_hline(yintercept = 0, 
    colour = "grey85", linetype = 2, alpha = 0.5) + geom_hline(yintercept = 5, colour = "white", 
    linetype = 1) + geom_hline(yintercept = 10, colour = "white", linetype = 1) + 
    geom_hline(yintercept = 15, colour = "white", linetype = 1) + geom_hline(yintercept = 20, 
    colour = "white", linetype = 1) + geom_hline(yintercept = 25, colour = "white", 
    linetype = 1) + geom_hline(yintercept = 30, colour = "white", linetype = 1) + 
    geom_hline(yintercept = 35, colour = "white", linetype = 1)
# plot the month seperator
dailyPlot <- dailyPlot + geom_vline(xintercept = 31, colour = "wheat4", linetype = 3, size = 0.5) + 
    geom_vline(xintercept = 59, colour = "wheat4", linetype = 3, size = 0.5) + geom_vline(xintercept = 90, 
    colour = "wheat4", linetype = 3, size = 0.5) + geom_vline(xintercept = 120, colour = "wheat4", 
    linetype = 3, size = 0.5) + geom_vline(xintercept = 151, colour = "wheat4", linetype = 3, 
    size = 0.5) + geom_vline(xintercept = 181, colour = "wheat4", linetype = 3, size = 0.5) + 
    geom_vline(xintercept = 212, colour = "wheat4", linetype = 3, size = 0.5) + geom_vline(xintercept = 243, 
    colour = "wheat4", linetype = 3, size = 0.5) + geom_vline(xintercept = 273, colour = "wheat4", 
    linetype = 3, size = 0.5) + geom_vline(xintercept = 304, colour = "wheat4", linetype = 3, 
    size = 0.5) + geom_vline(xintercept = 334, colour = "wheat4", linetype = 3, size = 0.5) + 
...
a <- dgr_fmt(seq(-30,40, by=5))

# Draw the x-axis month labels
dailyPlot <- dailyPlot + coord_cartesian(ylim = ylim.plot) + scale_y_continuous(breaks = seq(ylim.plot[1], 
    ylim.plot[2], by = 5), labels = a, expand = c(0, 0)) + scale_x_continuous(expand = c(0, 
    0), breaks = c(15, 45, 75, 105, 135, 165, 195, 228, 258, 288, 320, 350), labels = c("Januar", 
...
dailyPlot <- dailyPlot + geom_point(data = brokenLow, aes(x = yearDay, y = low.x), colour = "dodgerblue1", 
    shape = 21, fill = "dodgerblue1", size = 1.6) + geom_point(data = brokenHigh, 
