...
# Daten einlesen
dailyData <- read.table("data/dailyData.csv", header = TRUE, dec = ".", sep = ";") %>% 
...
dailyTemp <- ggplot(data = weatherDat, aes(x = Jahr, y = Temperatur_mw)) + geom_segment(data = tempsegDat, 
  ...
  theme(legend.position = "none") + scale_x_continuous(breaks = c(beginjahr, seq(1950, 
...
avgValues <- weatherDat %>% summarise_at(c("Temperatur_mw", "Niederschlag_summe"), 
  mean) %>% unlist()
dailyPrecip <- ggplot(data = weatherDat, aes(y = Temperatur_mw, x = Niederschlag_summe, 
  ...
  "black", "red"), breaks = c(0, 1), name = "Jahr", labels = c("1881-1999", paste0("2000-", 
