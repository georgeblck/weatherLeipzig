...
download.file("ftp://ftp-cdc.dwd.de/pub/CDC/observations_germany/climate/daily/kl/historical/tageswerte_KL_02932_19340101_20181231_hist.zip", 
    temp)
oldWeather <- read.csv(unz(temp, "produkt_klima_tag_19340101_20181231_02932.txt"), sep = ";", dec = ".", na.strings = c("NA", -999),
    header = TRUE)
unlink(temp)
...
newWeather <- read.csv(unz(temp, grep("^produkt", fileNames, value = TRUE)), sep = ";", dec = ".",  na.strings = c("NA", -999),
    header = TRUE)
unlink(temp)
