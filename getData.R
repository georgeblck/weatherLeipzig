...
weatherData <- read.table("weatherLeipzig.csv", dec = ".", sep = ";", 
                          header = TRUE, stringsAsFactors = FALSE)
# Format the Date
weatherData$Date <- ymd(weatherData$Date)

######################################### Get new data if the local one is outdated 
######################################### How outdated is the data
...
if (abs(days.outdated) > 1) {
  ...
  recentLink <- selectDWD(what.station, res = "daily", var = "kl", 
                          per = "recent")
  # Get the historic data (back to 1934)
  oldLink <- selectDWD(what.station, res = "daily", var = "kl", 
                       per = "historical")
  ...
  colnames(weatherData) <- c("Date", "QN3", "AVGWindtempo", 
                             "MaxWindtempo", "QN4", "NiederschlagMM", "NiederschlagsForm", 
                             "SonnenscheinStunden", "Schneehoehe", "NM", "AVGDampfDruck", 
                             "AVGLuftdruck", "AVGTemp", "AVGFeuchte", "MaxTemp2m", 
  ...
  write.table(weatherData, file = "weatherLeipzig.csv", dec = ".", 
              sep = ";", row.names = FALSE)
}