...
tempYear <- ggplot(data = saxonyMeans, aes(x = Jahr, y = Temp)) + geom_segment(aes(x = 1881, 
    ...
    scale_x_continuous(breaks = c(1881, seq(1900, 2018, by = 20), 2018)) + scale_y_continuous(expand = expand_scale(mult = 0.15))
...
tempPrecip <- ggplot(data = saxonyMeans, aes(x = Temp, y = Precip, color = factor(yearRegul))) + 
    ...
    xlab("Durchschnittstemperatur (°C)") + ylab("Niederschlag (mm)") + scale_color_manual(values = c("blue", 
    "black", "red"), name = "Jahr", labels = c("1881-1999", "2000-2017", "2018")) + 
    theme_tufte(base_size = 15) + geom_hline(yintercept = mean(saxonyMeans$Precip[saxonyMeans$yearRegul == 
    0]), linetype = "dashed", alpha = 0.5) + geom_segment(aes(x = mean(saxonyMeans$Temp[saxonyMeans$yearRegul == 
    0]), xend = mean(saxonyMeans$Temp[saxonyMeans$yearRegul == 0]), y = 400, yend = 900), 
    inherit.aes = FALSE, linetype = "dashed", alpha = 0.5, col = "black", data = data.frame()) + 
    scale_x_continuous(expand = expand_scale(mult = 0.15)) + scale_y_continuous(expand = expand_scale(mult = 0.15)) + 
    theme(legend.position = c(1, 1), legend.justification = c(1, 1), legend.background = element_rect(fill = "grey90", 
